...
# 生产环境建议从文件读取密钥，避免密钥出现在进程命令行中
wenyan serve --port 8080 --api-key-file /run/secrets/wenyan-api-key
```
...
| ---------------- | ---- | --------------------------------------------------------------------- | ---- | ------ |
| --port           | -    | 指定服务端口                                                          | 否   | 3000   |
| --api-key        | -    | 直接设置全局 API 密钥（开启鉴权），客户端请求需携带 `x-api-key` 头         | 否   | -      |
| --api-key-file   | -    | 从文件读取全局 API 密钥；生产环境推荐使用                              | 否   | -      |

`--api-key` 和 `--api-key-file` 不能同时使用。使用 `--api-key-file` 时，文件内容会去除首尾空白；文件不可读或内容为空时，Server 将拒绝启动，不会退化为无鉴权模式。
...
当服务启动时指定 `--api-key` 或 `--api-key-file` 参数，所有接口（除 `/health`）均需通过 API Key 认证：
